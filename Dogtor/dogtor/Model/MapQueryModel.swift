...
protocol MapQueryModelProtocol{
    func MapitemDownloaded(items: NSArray)
}

//class JsonModel:NSObject{
class MapQueryModel{
    var delegate: MapQueryModelProtocol!
    let urlPath = "http://\(myURL):8080/dogtor/hospital_list_query.jsp"
    ...
    func downloadItems() {
        ...
        let task = defaultSesstion.dataTask(with: url){(data, response, error) in
            if error != nil{
                ...
                print("Hospital Data is downloaded")
                self.parseJSON(data!)
            }
...
//                print(HospitalName, Lat, Long)
        ...
        DispatchQueue.main.async(execute: {() -> Void in
            self.delegate.MapitemDownloaded(items: locations)
        })
    }
}
